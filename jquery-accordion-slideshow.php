...
function JaS_install() 
{
	...
	if($wpdb->get_var("show tables like '". WpJqueryAccordionSlidshowTbl . "'") != WpJqueryAccordionSlidshowTbl) 
	{
		...
		$sSql = $IsSql . " VALUES ('wp-content/plugins/jquery-accordion-slideshow/gallery1/', 'GALLERY1', '6000', '230', '150', '200', '150', '27', 'YES', '1200', 'click', 'true', 'false', 'null', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
...
add_filter('the_content','JaS_Show_Filter');

function JaS_Show_Filter($content)
{
	return 	preg_replace_callback('/\[JQUERY-ACCORDION:(.*?)\]/sim','JaS_Show_Filter_Callback',$content);
}

function JaS_Show_Filter_Callback($matches) 
{
	global $wpdb;
	$Jas = "";
	$scode = $matches[1];
	//[JQUERY-ACCORDION:GALLERY1]
...
add_action('init', 'JaS_add_javascript_files');
